...
function initializeElements() {
  ...
  elements.skipDelayBtn = document.getElementById('skip-delay-btn');
...
async function loadInitialState() {
  try {
    ...
    // Premium UI: Restore draft prompt
    chrome.storage.local.get('draftPrompt', (res) => {
      if (res.draftPrompt && elements.promptInput) {
        elements.promptInput.value = res.draftPrompt;
      }
    });
  } catch (error) {
...
function attachEventListeners() {
  ...
  // Premium UI: Retain draft input on click-away
  elements.promptInput.addEventListener('input', (e) => {
    chrome.storage.local.set({ draftPrompt: e.target.value });
  });
  ...
  if (elements.skipDelayBtn) {
    elements.skipDelayBtn.addEventListener('click', () => {
      notifyServiceWorker('SKIP_DELAY');
    });
  }
...
function lockActiveQueueItem(isLocked) {
  ...
  if (firstItem) {
    if (isLocked) {
      firstItem.classList.add('active-processing');
    } else {
      firstItem.classList.remove('active-processing');
    }
  }
}
...
async function requestCurrentStatus() {
  try {
    ...
    if (response && response.success && response.data) {
      ...
      if (data.processingState) updateStatusIndicator(data.processingState);
      
      if (data.activeCountdown && data.processingState === 'sending_prompt') {
        handleStateUpdate({ type: 'COUNTDOWN_UPDATE', ...data.activeCountdown });
      }
      
      // Hydrate missing variables modal
      if (data.processingState === 'awaiting_user_input' && data.lastNeededPrompts) {
        handleStateUpdate({ type: 'PROMPT_NEEDS_INPUT', neededPrompts: data.lastNeededPrompts });
      }
      
      // Hydrate error/rate limit modal
      if (data.processingState === 'paused_for_error' && data.lastErrorPayload) {
        handleStateUpdate({ type: 'PAUSED_FOR_ERROR', ...data.lastErrorPayload });
      }
      
      if (typeof data.autoSendEnabled === 'boolean') {
        currentSettings.autoSendEnabled = data.autoSendEnabled;
        updateSettingsUI(currentSettings);
      }
    }
  } catch (error) {
  ...
  switch (payload.type) {
    ...
    case 'COUNTDOWN_UPDATE':
      if (elements.progressContainer) {
        const PREMIUM_CONTEXTS = {
          'Formulating': '<strong>Reason:</strong> Bypassing bot-detection algorithms.<br><strong>Action:</strong> Simulating human cognitive delay to ensure prompt acceptance and platform stability.',
          'Executing': '<strong>Reason:</strong> Respecting strict AI rate limits.<br><strong>Action:</strong> Holding execution to guarantee delivery without triggering an account throttle.'
        };
        ...
        const rM = Math.floor(payload.remaining / 60);
        const rS = payload.remaining % 60;
        const tM = Math.floor(payload.total / 60);
        const tS = payload.total % 60;
        
        elements.progressTime.textContent = `${rM > 0 ? rM + 'm ' : ''}${rS}s remaining of ${tM > 0 ? tM + 'm ' : ''}${tS}s`;
        
        if (descriptionEl) {
          descriptionEl.innerHTML = PREMIUM_CONTEXTS[payload.phase] || '<strong>Action:</strong> Processing queue pacing requirements.';
        }
...
function renderVariableModal(payload) {
  ...
  chrome.storage.local.get('draftVariables', (res) => {
    const draftVars = res.draftVariables || {};
    ...
    <div class="prompt-group" style="margin-bottom: var(--spacing-lg); padding-bottom: var(--spacing-sm); border-bottom: ${index !== uniqueVars.length - 1 ? '1px solid var(--color-border-light)' : 'none'};">
      <div class="variable-input-group">
        ...
        <input type="text" id="var-${escapeHtml(vData.name)}" name="${escapeHtml(vData.name)}" class="prompt-input" value="${escapeHtml(draftVars[vData.name] || '')}" required autocomplete="off" style="min-height: 36px; padding: var(--spacing-sm) var(--spacing-md);">
      </div>
    </div>
  `).join('');
  ...
  if (firstInput) setTimeout(() => firstInput.focus(), 50);
    ...
    // Premium UI: Attach listener to save variable drafts
    container.querySelectorAll('.prompt-input').forEach(input => {
      input.addEventListener('input', () => {
        const currentDrafts = {};
        const formData = new FormData(form);
        for (const [key, value] of formData.entries()) currentDrafts[key] = value;
        chrome.storage.local.set({ draftVariables: currentDrafts });
      });
    });
  });
  ...
  form.onsubmit = (e) => {
    ...
    chrome.storage.local.remove('draftVariables'); // Clear drafts on success
    modal.hidden = true;
...
async function handleQueueItemDoubleClick(e) {
  ...
  if (itemEl.classList.contains('active-processing')) {
    showStatusMessage('Cannot edit a prompt while it is processing.', 'warning');
    return;
  }
