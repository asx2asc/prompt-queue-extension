...
function setupMessageListener(handler) {
  const listener = (message, sender, sendResponse) => {
    ...
    if (message.type === 'ABORT_PROCESSING') {
      ...
      window.PromptQueueCommon.isAborted = true;
      log.warn('Received immediate ABORT_PROCESSING signal from Service Worker.');
      sendResponse({ success: true, aborted: true });
      return true;
    }

    if (message.type === 'SKIP_COUNTDOWN') {
      window.PromptQueueCommon.isSkipped = true;
      log.info('Received SKIP_COUNTDOWN signal from Service Worker.');
...
function sleepWithCountdown(ms, phaseName) {
  return new Promise((resolve, reject) => {
    window.PromptQueueCommon.isSkipped = false; // Reset skip flag
    ...
    const interval = setInterval(() => {
      if (window.PromptQueueCommon.isAborted) {
        clearInterval(interval);
        reject(new Error('Aborted by user Kill Switch.'));
        return;
      }
      if (window.PromptQueueCommon.isSkipped) {
        clearInterval(interval);
        log.info(`Countdown for ${phaseName} skipped by user.`);
        resolve(); // Safely bypass delay without failing
        return;
      }
...
async function retry(fn, options = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    if (window.PromptQueueCommon && window.PromptQueueCommon.isAborted) {
      throw new Error('Aborted by user Kill Switch.');
    }
    try {
      return await fn();
    } catch (error) {
