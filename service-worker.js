...
class StateManager {
  ...
  async initialize() {
    try {
      ...
      if (stored.settings) {
        this._state.set('settings', stored.settings, false);
        if (typeof stored.settings.autoSendEnabled === 'boolean') {
...
async function notifyPopup(data) {
  // Centralized intercept to ensure volatile error states are cached for rehydration
  if (data.type === 'PAUSED_FOR_ERROR') {
    stateManager.set('lastErrorPayload', { errorType: data.errorType, message: data.message }, false);
  }
...
function getCurrentStatus() {
  ...
  return {
    ...
    activeCountdown: stateManager.get('activeCountdown') || null,
    lastNeededPrompts: stateManager.get('lastNeededPrompts') || null,
    lastErrorPayload: stateManager.get('lastErrorPayload') || null
  };
}
...
class MessageRouter {
  ...
  async _handlePopupMessage(type, payload) {
    switch (type) {
      ...
      case 'SKIP_DELAY': {
        logger.info('Skip delay explicitly requested');
        const targetTabId = focusManager.getProcessingTab() || stateManager.get('currentTabId');
        if (targetTabId) {
          sendToContentScript(targetTabId, { type: 'SKIP_COUNTDOWN' }).catch(() => {});
        }
        return { skipped: true };
      }
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  if (changes.settings) {
    ...
    stateManager.set('settings', newSettings, false);
    if (typeof newSettings.autoSendEnabled === 'boolean') {
      stateManager.set('autoSendEnabled', newSettings.autoSendEnabled, false);
    }
    logger.info('Settings synced from storage:', newSettings);
  }
});
